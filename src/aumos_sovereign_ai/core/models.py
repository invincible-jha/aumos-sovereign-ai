...
class SovereignModel(AumOSModel):
    ...
    # GAP-343: EU AI Act risk classification
    eu_ai_act_risk_tier: Mapped[str | None] = mapped_column(
        String(20),
        nullable=True,
        index=True,
        comment="EU AI Act 2024/1689 risk tier: unacceptable, high, limited, minimal",
    )
    eu_ai_act_classification_json: Mapped[dict | None] = mapped_column(
        JSONB,
        nullable=True,
        comment="Full EUAIActClassificationResult JSON including conformity assessment evidence",
    )
    # GAP-342: Gaia-X credential reference
    gaia_x_credential_id: Mapped[str | None] = mapped_column(
        String(36),
        nullable=True,
        comment="UUID of associated GaiaXCredential — null if not Gaia-X registered",
    )


class GaiaXCredential(AumOSModel):
    """Gaia-X Trust Framework Verifiable Credential record.

    Stores generated W3C VC 1.1 self-descriptions for Gaia-X Participant
    and Service Offering credentials. Linked to SovereignModel registrations.

    Table: sov_gaia_x_credentials
    """

    __tablename__ = "sov_gaia_x_credentials"

    credential_type: Mapped[str] = mapped_column(
        String(30),
        nullable=False,
        index=True,
        comment="Credential type: participant or service_offering",
    )
    credential_json: Mapped[dict] = mapped_column(
        JSONB,
        nullable=False,
        default=dict,
        comment="Full W3C VC 1.1 credential document as generated by GaiaXAdapter",
    )
    catalogue_id: Mapped[str | None] = mapped_column(
        String(255),
        nullable=True,
        comment="Gaia-X Federated Catalogue assigned identifier after registration",
    )
    signing_key_id: Mapped[str] = mapped_column(
        String(255),
        nullable=False,
        comment="Signing key ID used for this credential (references HSM key — GAP-346)",
    )
    expiration_date: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="ISO 8601 expiration date from the credential issuanceDate + 365 days",
    )
    is_registered: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
        comment="True once successfully registered with the Gaia-X Federated Catalogue",
    )
